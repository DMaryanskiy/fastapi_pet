...
async def get_lists(
    ...
    lists = result.all()
    resp = [schemas.List(**item._asdict()) for item in lists]
    return resp
...
async def retrieve_list(
    ...
    result: AsyncResult = await session.execute(query_retrieve)
    list_item = result.one()

    resp = schemas.List(**list_item._asdict())
